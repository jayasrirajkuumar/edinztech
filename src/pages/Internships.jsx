...
export default function Internships() {
    ...
    useEffect(() => {
        const fetchPrograms = async () => {
            try {
                const data = await getProgramsByType('Internship');
                setPrograms(data);
            } catch (err) {
    ...
    const filteredPrograms = useMemo(() => {
        return programs.filter(program => {
            ...
            // 4. Duration Filter
            if (duration !== 'All') {
                // Approximate logic based on days.
                let days = 0;
                ...
                if (duration === 'short' && days > 14) return false; // 1-2 weeks
                if (duration === 'medium' && (days <= 14 || days > 45)) return false; // ~1 month
                if (duration === 'long' && days <= 45) return false; // > 1 month
            }
    ...
    const tabs = [
        ...
        { id: 'Upcoming', label: 'Upcoming' }, // Swapped order for better UX
    ...
    return (
        <div className="min-h-screen bg-gray-50 pt-24 pb-12">
            <div className="max-w-7xl mx-auto px-4">
                <div className="bg-purple-900 rounded-2xl p-8 mb-8 text-white relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div className="relative z-10 max-w-2xl">
                        <span className="inline-block px-3 py-1 bg-purple-800 rounded-full text-sm font-semibold mb-4 border border-purple-700">
                            Career Launchpad
                        </span>
                        <h1 className="text-4xl md:text-5xl font-bold mb-4">Industry Internships</h1>
                        <p className="text-purple-100 text-lg">
                            Gain real world experience working on live projects with mentorship.
                        </p>
                    </div>
                    <Icons.Internships className="absolute right-0 bottom-0 text-white/10 w-64 h-64 translate-x-12 translate-y-12 rotate-[-15deg]" />
                </div>
                ...
                <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        <select
                            ...
                            className="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 outline-none focus:border-purple-500"
                        >
